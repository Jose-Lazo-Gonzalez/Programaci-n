ventas=[["Portátil", 150, 799.99, 4.5],
        ["Smartphone", 250, 599.99, 4.3],
        ["Auriculares", 400, 49.99, 4.0],
        ["Tablet", 120, 299.99, 3.9],
        ["Monitor", 180, 199.99, 4.2],
        ["Smartwatch", 220, 149.99, 4.1],
        ["Teclado mecánico", 300, 89.99, 4.4],
        ["Ratón gaming", 350, 59.99, 4.0],
        ["Cámara digital", 90, 999.99, 4.6],
        ["Consola", 200, 399.99, 4.7]]

#Apartado1
def getProducto(ventas, nombreProducto):
    producto=[]
    i=0
    encontrado = False
    while i < len(ventas) and not encontrado:
        if ventas[i][0] == nombreProducto:
            producto = ventas[i]
            encontrado = True
        else:
            i+=1
    return producto

def calculaIngresos(ventas, nombreProducto):
    filaProducto = getProducto(ventas, nombreProducto)
    if filaProducto:
        precio=filaProducto[1]*filaProducto[2]
    return precio

#Apartado2
def productoDestacado(ventas, nombreProducto):
    valoracionMedia = getProducto(ventas, nombreProducto)
    devuelve = False
    if len(valoracionMedia)>0:
        if valoracionMedia[3]>=4.2:
            devuelve = True
        else:
            devuelve = False
    return devuelve

#Apartado2.1
def getProductoDestacado(ventas):
    lista=[]
    for elemento in ventas:
        if productoDestacado(ventas, elemento[0]):
            lista.append(elemento)
    return lista

#apartado 3
def tieneMayorIngreso (nombreProducto, nombreProducto2):
    
    if calculaIngresos (ventas,nombreProducto)>calculaIngresos(ventas,nombreProducto2):
        return True
    else: 
        return False

def calcularIngresoTotales(ventas):
    precioTotal=0
    for elemento in ventas:
        filaProducto=elemento
        precio=filaProducto[1]*filaProducto[2]
        precioTotal+=precio
    return precioTotal

nombreProducto=input("dime el nombre de un producto:")
print(getProducto(ventas, nombreProducto))
print(calculaIngresos(ventas, nombreProducto))
print(productoDestacado(ventas, nombreProducto))
print(getProductoDestacado(ventas))
nombreProducto2=input("dime el nombre del producto a comparar:")
print(calculaIngresos(ventas, nombreProducto2))
print(tieneMayorIngreso (nombreProducto, nombreProducto2))
assert calcularIngresoTotales(ventas)==612977.4
print( calcularIngresoTotales(ventas))
